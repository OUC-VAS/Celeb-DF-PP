...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        try:
            ...
            # with open(os.path.join(self.config['dataset_json_folder'], dataset_name + '-' +self.config['json_name']), 'r') as f:
        ...
        if dataset_name.startswith('Celeb-DF-v3-'): # for protocol #3
            dataset_name = 'Celeb-DF-v3'
        if dataset_name != 'Celeb-DF-v3':
                ...
                if label == 'CelebDFv3_real':
                    ...
                    for video_name, video_info in sub_dataset_info.items():
                        ...
                        if self.frame_num < total_frames:
                            ...
                            if self.video_level:
                                # Select clip_size continuous frames
                                start_frame = random.randint(0,
                                                             total_frames - self.frame_num) if self.mode == 'train' else 0
                                frame_paths = frame_paths[start_frame:start_frame + self.frame_num]  # update total_frames
                            else:
                                # Select self.frame_num frames evenly distributed throughout the video
                                step = total_frames // self.frame_num
                        ...
                        if self.video_level:
                            ...
                            if total_frames >= self.clip_size:
                                ...
                                # Append the list of selected clips and append the label
                                label_list.extend([label] * len(selected_clips) * len(dataset_info[dataset_name]['CelebDFv3_fake']['test']))
                                frame_path_list.extend(selected_clips * len(dataset_info[dataset_name]['CelebDFv3_fake']['test']))
                                # video name save
                                video_name_list.extend([unique_video_name] * len(selected_clips) * len(dataset_info[dataset_name]['CelebDFv3_fake']['test']))
                        ...
                        # Otherwise, extend the label and frame paths to the lists according to the number of frames
                            ...
                            if self.mode == 'train':    # *3 is to make up for the insufficient number of real training sets (cdfv2)
                                # Extend the label and frame paths to the lists according to the number of frames
                                label_list.extend([label] * total_frames * len(dataset_info[dataset_name]['CelebDFv3_fake']['train']) * 3)
                                frame_path_list.extend(frame_paths * len(dataset_info[dataset_name]['CelebDFv3_fake']['train']) * 3)
                                # video name save
                                video_name_list.extend([unique_video_name] * len(frame_paths) * len(dataset_info[dataset_name]['CelebDFv3_fake']['train']) * 3)
                            else:
                                label_list.extend([label] * total_frames)
                                frame_path_list.extend(frame_paths)
                                # video name save
                                video_name_list.extend([unique_video_name] * len(frame_paths))
                elif label == 'CelebDFv3_fake':
                    for method in list(dataset_info[dataset_name][label][self.mode].keys()):
                        sub_dataset_info = dataset_info[dataset_name]['CelebDFv3_fake'][self.mode][method]
                        for video_name, video_info in sub_dataset_info.items():
                            # Unique video name
                            unique_video_name = video_info['label'] + '_' + video_name
                            # unique_video_name = video_info['label'] + '_' + method + '_' + video_name
