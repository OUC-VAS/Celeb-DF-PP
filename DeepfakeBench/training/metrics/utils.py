...
def get_test_metrics(y_pred, y_true, img_names, dataset_name=None):
    def get_video_metrics(image, pred, label, dataset_name):
        result_dict = {}
        new_label = []
        new_pred = []
        # print(image[0])
        ...
        for item in np.transpose(np.stack((image, pred, label)), (1, 0)):
            ...
            if "Celeb-DF-v3" in s and "Celeb-synthesis" in s:
                c = parts[-4]
                d = c + '_' + a
                if d not in result_dict:
                    result_dict[d] = []
                result_dict[d].append(item)
            else:
        ...
        for video in image_arr:
            ...
            for frame in video:
                ...
                leng += 1
            if "Celeb-DF-v3" in frame[0] and "real" in frame[0]:
                if dataset_name == 'Celeb-DF-v3':
                    iter = 22
                else:
                    iter = 1
                for _ in range(iter):     # number of methods
    ...
    if type(img_names[0]) is not list:
        ...
        v_auc, _ = get_video_metrics(img_names, y_pred, y_true, dataset_name)
    else:
        # video-level methods
        v_auc=auc
