...
def generate_dataset_file(dataset_name, dataset_root_path, output_file_path, compression_level='c23', perturbation = 'end_to_end'):
    ...
    ## Celeb-DF-v3 dataset
    ## Note: videos in Celeb-DF-v1/2/3 are not in the same format as in FaceForensics++ dataset
        ...
        for folder in os.scandir(dataset_path):
            ...
            assert label in ['CelebDFv3_real', 'CelebDFv3_fake'], 'Invalid label: {}'.format(label)
            if folder.name != 'Celeb-real':
                dataset_dict[dataset_name][label] = {}
                dataset_dict[dataset_name][label]['train'] = {}
                dataset_dict[dataset_name][label]['val'] = {}
                dataset_dict[dataset_name][label]['test'] = {}
            if folder.name in ['Celeb-real', 'YouTube-real']:
                ...
                manipulation1 = ['Celeb-DF-v2', 'BlendFace', 'GHOST', 'HifiFace', 'InSwapper', 'MobileFaceSwap', 'SimSwap', 'UniFace']
                manipulation2 = ['DaGAN', 'FSRT', 'HyperReenact', 'LIA', 'LivePortrait', 'MCNET', 'TPSMM']
                manipulation3 = ['AniTalker', 'EchoMimic', 'EDTalk', 'FLOAT', 'IP_LAP', 'Real3DPortrait', 'SadTalker']
                FaceSwap = [os.path.join('FaceSwap', method) for method in manipulation1]
                FaceReenact = [os.path.join('FaceReenact', method) for method in manipulation2]
                TalkingFace = [os.path.join('TalkingFace', method) for method in manipulation3]
                all_manipulations = FaceSwap + FaceReenact + TalkingFace
                # all_manipulations = TalkingFace[3:]
                
                for manipulation in all_manipulations:
                    dataset_dict[dataset_name][label]['train'][manipulation.split('/')[1]] = {}
                    dataset_dict[dataset_name][label]['test'][manipulation.split('/')[1]] = {}
                    dataset_dict[dataset_name][label]['val'][manipulation.split('/')[1]] = {}
                    for video_path in os.scandir(os.path.join(dataset_path, folder.name, manipulation, 'frames')):
                        if video_path.is_dir():
                            video_name = video_path.name
                            frame_paths = [os.path.join(video_path, frame.name) for frame in os.scandir(video_path)]
                            # dataset_dict[dataset_name][label]['train'][video_name] = {'label': label, 'frames': frame_paths}
                            dataset_dict[dataset_name][label]['train'][manipulation.split('/')[1]][video_name] = {'label': label, 'frames': frame_paths}
        # Special case for test&val data of Celeb-DF-v1/2
        with open(os.path.join(dataset_root_path, dataset_name, 'List_of_testing_videos.txt'), 'r') as f:
        # with open(os.path.join(dataset_root_path, dataset_name, 'random_auc_5.txt'), 'r') as f:
            lines = f.readlines()
        for line in lines:
            if 'real' in line:
                label = 'CelebDFv3_real'
            ...
            if 'real' in line:
                ...
                dataset_dict[dataset_name][label]['test'][vidname] = {'label': label, 'frames': frame_paths}
                dataset_dict[dataset_name][label]['val'][vidname] = {'label': label, 'frames': frame_paths}
                del dataset_dict[dataset_name][label]['train'][vidname] 
            else:
                label_fake = line.split('\n')[0].split('/')[-2]
                vidname = line.split('\n')[0].split('/')[-1].split('.mp4')[0]
                frame_paths = glob.glob(
                    os.path.join(dataset_root_path, dataset_name, '/'.join(line.split(' ')[1].split('/')[:3]), 'frames', vidname,
                                 '*png'))
                dataset_dict[dataset_name][label]['test'][label_fake][vidname] = {'label': label, 'frames': frame_paths}
                dataset_dict[dataset_name][label]['val'][label_fake][vidname] = {'label': label, 'frames': frame_paths}
                del dataset_dict[dataset_name][label]['train'][label_fake][vidname] 
    ...
    # for 22 deepfake methods testing -> json file and train cdfv2 test other 21 methods
    # for deepfake_name in manipulation1 + manipulation2 + manipulation3:
    #     output_file_path = config['rearrange']['output_file_path']['default']
    #     import copy
    #     dict_deepfake = copy.deepcopy(dataset_dict)
    #     for key in list(dict_deepfake[dataset_name]['CelebDFv3_fake']['train'].keys()):
    #         if key != deepfake_name:
    #             del dict_deepfake[dataset_name]['CelebDFv3_fake']['train'][key]
    #             del dict_deepfake[dataset_name]['CelebDFv3_fake']['val'][key]
    #             del dict_deepfake[dataset_name]['CelebDFv3_fake']['test'][key]
    #     output_file_path = os.path.join(output_file_path, dataset_name + '-' + deepfake_name + '.json')
    #     with open(output_file_path, 'w') as f:
    #         json.dump(dict_deepfake, f)
    #     print(f"{deepfake_name}.json generated successfully.")

if __name__ == '__main__':
    # from config.yaml load parameters
    yaml_path = '/data/zhudelong/DeepfakeBench-main/preprocessing/config.yaml'
    # open the yaml file
    try:
